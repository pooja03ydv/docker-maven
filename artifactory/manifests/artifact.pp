...
#
# @example
 #  artifactory::artifact {'/opt/mavenartifacts/artifacts.jar:
 #    group_id    => 'org.sonarqube',
 #    artifact_id => 'sonarscanner-maven-basic',
 #    version     => '',
 #    repository  => 'example-repo-local/mavenartifacts',
 #    output      => "/tmp/",
 #    extension   => 'jar',
 #  }
#
...
define artifactory::artifact (
  ...
  $cmd = $facts['os']['family'] ? {
    ...
    default         => "curl -k  \"${$artifact[download_uri]}\"  -o ${target_file}  \"${auth}\"",
  }
